...
import eu.xfsc.tsa.sign.resolve.DidDocumentResolver;
...
public class UniSignatureVerifier implements SignatureVerifier {
	...
	private boolean verifyLDProof(JsonLDObject payload, LdProof proof, String did, String alg) {
		...
		Optional<VerificationMethod> ovm = vrMethods.stream().filter(vm -> {
			...
			try {
				JWK jwk = JWK.fromMap(vm.getPublicKeyJwk());
				// not sure..
				if (jwk.getAlg() == null) {
					jwk.setAlg(alg);
				}
	...
	public boolean verify(JsonLDObject payload, LdProof proof, JWK jwk, String alg) {
	    ...
	    try {
			return verifier.verify(payload, proof);
		} catch (IOException | GeneralSecurityException | JsonLDException ex) {
			log.info("verify.error: {}", ex.getMessage(), ex);
		}
		return false;
	}
