...
import eu.xfsc.tsa.sign.exception.DidException;
import eu.xfsc.tsa.sign.verify.LocalSignatureVerifier;
import eu.xfsc.tsa.sign.verify.SignatureVerifier;
import eu.xfsc.tsa.sign.verify.UniSignatureVerifier;
...
public class DidResolverConfig {
    ...
    @Value("${signer.verification.signature-verifier}")
    private String signatureVerifier;

	@Bean
	public UniResolver uniResolver(@Value("${signer.verification.did.base-url}") String baseUrl) {
	...
	@Bean
	public Cache<String, DIDDocument> didDocumentCache(@Value("${signer.verification.did.cache.size}") int cacheSize,
			@Value("${signer.verification.did.cache.timeout}") Duration timeout) {
